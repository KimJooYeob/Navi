...
public class MainActivity extends BaseActivity implements onLocationChangedCallback
{
	...
	public static String mApiKey="694bdacd-3481-3345-8811-2ddab000f50c"; // 발급받은 appKey
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		GPSReciever location=new GPSReciever("192.168.43.90");//IP주소
		location.start();
		while(!location.getState().equals(Thread.State.TERMINATED));
		
		double startLatitude = location.getFromLatitude();
		double startLongitude = location.getFromLongitude();
		double endLatitude = location.getToLatitude();
		double endLongitude = location.getToLongitude();
		
	
		
	
		drawPedestrianPath(startLatitude, startLongitude, endLatitude, endLongitude);
	}
	...
	private void configureMapView() {
		...
		System.out.println("mBizMapID : "  + mBizAppID);
	}
	...
	public void animateTo(TMapPoint point) {
...
/*	public void setCompassMode() {
	...
	{			
		mMapView.setCenterPoint(36.365464, 127.3464118);

		TMapPoint point1 = mMapView.getCenterPoint();
		TMapPoint point2 = new TMapPoint(36.357146, 127.343075);
	...
	public void  drawPedestrianPath(double startLatitude, double startLongitude, double endLatitude, double endLongitude)
	{				
		mMapView.setCenterPoint(startLongitude,startLatitude);

		TMapPoint point1 = mMapView.getCenterPoint();
		TMapPoint point2 = new TMapPoint(endLatitude, endLongitude);
		TMapData tmapdata = new TMapData();
		...
		tmapdata.findPathDataWithType(TMapPathType.PEDESTRIAN_PATH, point1, point2, new FindPathDataListenerCallback() {
			...
			@Override
			public void onFindPathData(TMapPolyLine polyLine) {
				System.out.println("onFindPathData");
				polyLine.setLineColor(Color.BLUE);
				mMapView.addTMapPath(polyLine);
